...
#let personalised-nutrition-guidelines(section, report) = page[
  ...
  #let food-categories-zh-HK = yaml("food-categories-zh-HK.yaml")
  ...
  #let food-recommendation(title: [], category: "") = [
    ...
    #let recommendations-aggregate(category, tier) = align(horizon, stack(dir: ltr, [#i18n.at(tier).zh_HK: ], pad(
      x: 8pt,
      circle(
        ...
        .map(((i, food)) => {
          grid(
            ...
            box(height: 45pt, align(bottom, stack(
              text(size: if food-categories-zh-HK.at(category).at(i).len() > 15 { 10pt } else { 14pt })[#par(
                leading: 0.4em,
                food-categories-zh-HK.at(category).at(i),
              )],
              v(0.5em),
